...
- **2026-09-16 — quant-dedup + friend glossary encoded.** Workflows name different people's quants of the same base model: agent reuses ONE canonical file per role (realrebelai default) and widget-swaps workflows to it, never downloads the same role twice (drive skill §Quant dedup, MODELS.md §Canonical picks). Friend knows only in/out + TV-static metaphor: HU comms got a nickname glossary (videó-agy/rajzoló, szövegértő, képösszerakó, tömörített változat, gyorsító, felnagyító, hangagy) — tech terms never unexplained; UI guidance = nickname + filename + where to click.
